...
const createMenu = (objeto) => {
  meuRestaurante = {
    ...
    pay: () => {
      ...
      const { food } = objeto;
      const { drink } = objeto;
      for (let i = 0; i < meuRestaurante.consumption.length; i += 1) {
        total += food[meuRestaurante.consumption[i]] > 0 ? food[meuRestaurante.consumption[i]]
          : drink[meuRestaurante.consumption[i]];
      }
      return total;
    },
  };
